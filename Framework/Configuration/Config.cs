...
namespace Framework.Configuration
{
    public class Config
    {
        IList<string> ConfigContent;
        public string ConfigFile { get; set; }

        public Config(string config)
        {
            ConfigFile = config;
            GetConfigContentAsync();
        }

        protected async void GetConfigContentAsync()
        {
            StorageFile confFile = await ApplicationData.Current.LocalFolder.GetFileAsync(ConfigFile);
            if (confFile != null)
                ConfigContent = await FileIO.ReadLinesAsync(confFile);
            else
                Log.Message(LogType.ERROR, "{0} doesn't exist!", ConfigFile);
        }
        ...
        public T Read<T>(string name, T value)
        {
            ...
            try
            {
                foreach (var option in ConfigContent)
                {
                    ...
                    if (configOption[0].StartsWith(name, StringComparison.CurrentCulture))
