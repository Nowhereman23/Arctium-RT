...
namespace Framework.Cryptography
{
    public class SRP6
    {
        static HashAlgorithmProvider hashAlgorithm = HashAlgorithmProvider.OpenAlgorithm(HashAlgorithmNames.Sha1);
        ...
        public byte[] ComputeHash(byte[] value)
        {
            ...
            var iHash = hashAlgorithm.HashData(binValue);

            CryptographicBuffer.CopyToByteArray(iHash, out hash);
        ...
        {
            List<byte[]> list = Split(this.S);

            list[0] = ComputeHash((byte[])list[0]);
            list[1] = ComputeHash((byte[])list[1]);
        ...
        public void CalculateM2(byte[] m1)
        {
            ...
            System.Buffer.BlockCopy(this.A, 0, dst, 0, this.A.Length);
            System.Buffer.BlockCopy(m1, 0, dst, this.A.Length, m1.Length);
            System.Buffer.BlockCopy(this.K, 0, dst, this.A.Length + m1.Length, this.K.Length);
            this.M2 = ComputeHash(dst);
        }
        ...
        public void CalculateU(byte[] a)
        {
            ...
            System.Buffer.BlockCopy(a, 0, dst, 0, a.Length);
            System.Buffer.BlockCopy(this.B, 0, dst, a.Length, this.B.Length);
            this.U = ComputeHash(dst);
        ...
        public void CalculateX(byte[] username, byte[] password)
        {
            ...
            dst[src.Length] = 0x3a;
            System.Buffer.BlockCopy(buffer2, 0, dst, src.Length + 1, buffer2.Length);
            System.Buffer.BlockCopy(ComputeHash(dst), 0, buffer5, this.salt.Length, 20);
            System.Buffer.BlockCopy(this.salt, 0, buffer5, 0, this.salt.Length);
            byte[] array = ComputeHash(buffer5);
            Array.Reverse(array);
        ...
        private static List<byte[]> Split(byte[] bo)
        {
            ...
            {
                System.Buffer.BlockCopy(bo, 1, dst, 0, bo.Length);
            }
